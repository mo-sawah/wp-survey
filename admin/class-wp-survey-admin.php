...
class WP_Survey_Admin {
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page('wp-survey', __('All Surveys', 'wp-survey'), __('All Surveys', 'wp-survey'),
            'manage_options', 'wp-survey', [$this, 'render_surveys_page']);
        
        add_submenu_page('wp-survey', __('Add New', 'wp-survey'), __('Add New', 'wp-survey'),
            'manage_options', 'wp-survey-add', [$this, 'render_add_edit_page']);
        
        add_submenu_page('wp-survey', __('Analytics', 'wp-survey'), __('📊 Analytics', 'wp-survey'),
            'manage_options', 'wp-survey-analytics', [$this, 'render_analytics_page']);
        
        add_submenu_page('wp-survey', __('Emails', 'wp-survey'), __('Emails', 'wp-survey'),
            'manage_options', 'wp-survey-emails', [$this, 'render_emails_page']);
        
        add_submenu_page('wp-survey', __('Import / Export', 'wp-survey'), __('Import / Export', 'wp-survey'),
            'manage_options', 'wp-survey-import-export', [$this, 'render_import_export_page']);
    }
    ...
    public function enqueue_admin_assets($hook) {
        ...
        // Enqueue Chart.js only on analytics page
        if (strpos($hook, 'wp-survey-analytics') !== false) {
    ...
    public function render_analytics_page() {
        ...
        // Default to first survey if none selected
    ...
    private function handle_import() {
        ...
        if ($survey_data['survey_type'] === 'multi-question') {
            foreach ($data['questions'] as $q_index => $question) {
                ...
                if ($q_id && isset($question['choices'])) {
        ...
        if ($survey->survey_type === 'multi-question') {
            ...
            foreach ($questions as $q) {
                ...
                foreach ($choices as $c) {
                    $choices_array[] = ['title' => $c->title, 'description_1' => $c->description_1,
            ...
            $export_data['choices'] = [];
            foreach ($choices as $c) {
                $export_data['choices'][] = ['title' => $c->title, 'description_1' => $c->description_1,
                                              'description_2' => $c->description_2, 'image_url' => $c->image_url];
            }
        }
